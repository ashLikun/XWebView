...
public class WebProxyConfig {
    public static final String FILE_CACHE_PATH = File.separator + "webproxy-cache";
    /**
     * 缓存路径，可以更改
     */
    public static String WEBPROXY_CHCHE_PATH;
    ...
    private static final String TAG = WebProxyConfig.class.getSimpleName();
    /**
     * AgentWeb 的版本
     */
    ...
    public static String getCachePath(Context context) {
        if (!TextUtils.isEmpty(WebProxyConfig.WEBPROXY_CHCHE_PATH)) {
            return WebProxyConfig.WEBPROXY_CHCHE_PATH;
        }
        String dir = context.getExternalCacheDir().getAbsolutePath();
        File mFile = new File(dir, WebProxyConfig.FILE_CACHE_PATH);
        try {
            if (!mFile.exists()) {
                mFile.mkdirs();
            }
        ...
        return WebProxyConfig.WEBPROXY_CHCHE_PATH = mFile.getAbsolutePath();
    }
    ...
    public static String getExternalCachePath(Context context) {
        return WebProxyUtils.getAgentWebFilePath(context);
    }
    ...
    public static synchronized void clearDiskCache(Context context) {
        ...
        try {
            ...
            WebProxyUtils.clearCacheFolder(new File(getCachePath(context)), 0);
            String path = getExternalCachePath(context);
            if (!TextUtils.isEmpty(path)) {
                File mFile = new File(path);
                WebProxyUtils.clearCacheFolder(mFile, 0);
            }
        } catch (Throwable throwable) {
        }
